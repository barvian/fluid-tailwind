...
description: Scale values between breakpoints without media queries.
...
* Fluid utilities scale between their start and end values (separated by `/`)
when the viewport is between the smallest and largest breakpoints, by default
* The default start/end breakpoints [can be customized](#custom-default-screens) or [overridden per-utility](#customize-screens-per-utility)

## Limitations

Due to CSS restrictions, fluid utilities only work if the start/end values and points are length literals (i.e. `1rem`) with the same unit.
...
<Tip.Bad>Values with different units than points</Tip.Bad>
...
Tailwind doesn't currently provide any error reporting tools for plugins, so in case a fluid utility fails,
it will output an empty rule with a comment in the selector:
...
The default start and end breakopints can be set with a tuple `[start, end]`, where each
value is a length literal.
...
You can omit either start or end breakpoint to use your [global defaults](#custom-default-screens):

<Tip.Good>Set start breakpoint to `md` (end breakpoint defaults to largest)</Tip.Good>
```html mark="~md:"
<div class="~md:~text-base/4xl">
```

<Tip.Good>Set end breakpoint to `lg` (start breakpoint defaults to smallest)</Tip.Good>
```html mark="~/lg:"
