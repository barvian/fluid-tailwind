...
title: Fluid for Tailwind CSS - Smoother responsive styles in less code.
...
<Steps.Root>
  ...
  <Steps.Item>
    ...
    <Fragment slot="code">
      ```js title="tailwind.config.js" ins={1,6,10} mark=/\: (\{)/ mark="files:" mark=/\](,)/ mark=/(\}),/
  ...
  <Steps.Item>
    <>
      ### Convert Tailwind's default screens to `rem`

      <Tip.Info>You can skip this if you [override Tailwind's default screens](https://tailwindcss.com/docs/screens#overriding-the-defaults) or use [`tw-reset`](https://github.com/barvian/tw-reset)</Tip.Info>

      This [will be unnecessary](https://github.com/tailwindlabs/tailwindcss/pull/13469) in Tailwind v4.
    </>
    <Fragment slot="code">
...
Tailwind's default font sizes starting at `5xl` use unitless line heights that can't
be fluidized. If you want to use these values fluidly, you can convert them to `rem` with the built-in `fontSize` export:

```js title="tailwind.config.js" ins={6} ins=", fontSize"
