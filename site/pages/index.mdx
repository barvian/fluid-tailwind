...
description: A clamp() plugin for Tailwind that works with every utility.
...
Due to CSS restrictions, fluid `clamp()` functions can only be generated if the start/end values and points are length literals (i.e. `1rem`) with the same unit.
...
Tailwind doesn't currently provide any error reporting tools for plugins, so in case a fluid utility
can't generate a `clamp()`, it will output an empty rule with a comment in the selector:
