...
* The default start/end breakpoints [can be customized](#custom-default-breakpoints) or [overridden per-utility](#customize-breakpoints-per-utility)

## Limitations

Due to CSS restrictions, fluid utilities only work if the start/end values and breakpoints are length literals (i.e. `1rem`) with the same unit.
...
The default start/end breakpoints can be set with a tuple `[start, end]`, where each
one is a length literal.
Either one can be omitted, in which case the plugin will use your smallest and largest
...
You can omit either start or end breakpoint to use your [global defaults](#custom-default-breakpoints):
...
Either one can be omitted, in which case the plugin will use your smallest and
